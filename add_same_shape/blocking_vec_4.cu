...
// store
__global__ void gpuReduce_block_size_store(float*x, float* y, int nx, int ny) {
   #pragma unroll
   for (int ix = 0; ix < block_size_x; ix++) {
     int idx_dst = blockIdx.x * blockDim.x * block_size_x + threadIdx.x * block_size_x + ix;
     y[idx_dst] = 1.0f;
...
// store
__global__ void gpuReduce_block_size_storeAll(float*x, float* y, int nx, int ny) {
   for (int iy = 0; iy < ny; iy++) {
     #pragma unroll
     for (int ix = 0; ix < block_size_x; ix++) {
       int idx_dst = iy * nx + blockIdx.x * blockDim.x * block_size_x + threadIdx.x * block_size_x + ix;
       if(idx_dst < nx * ny)
       y[idx_dst] = 1.0f;
     }
   }
...
// load
__global__ void gpuReduce_block_size_loadAndStore(float*x, float* y, int nx, int ny) {
   for (int iy = 0; iy < ny; iy++) {
     #pragma unroll
     for (int ix = 0; ix < block_size_x; ix++) {
       int idx_dst = iy * nx + blockIdx.x * blockDim.x * block_size_x + threadIdx.x * block_size_x + ix;
...
// register
__global__ void gpuReduce_vec_store(float*x, float* y, int nx, int ny) {
...
// register
__global__ void gpuReduce_vec_storeAll(float*x, float* y, int nx, int ny) {
   using Vec = aligned_vector<float, vecsize>;
   Vec *src = reinterpret_cast<Vec *>(x); 
   Vec *dst = reinterpret_cast<Vec *>(y); 
   int idx = blockIdx.x * blockDim.x + threadIdx.x;
   Vec temp;
...
__global__ void gpuReduce_vec_4(float*x, float* y, int nx, int ny) {
   ...
   //  }
    // #pragma unroll
    // for (int ix = 0; ix < 4; ix++) {
    //    temp.val[ix] += temp_1.val[ix]; 
    // }
     dst[t] = temp_1;
   }
}
// register
__global__ void gpuReduce_vec_loadAndStore(float*x, float* y, int nx, int ny) {
   using Vec = aligned_vector<float, vecsize>;
   Vec *src = reinterpret_cast<Vec *>(x); 
   Vec *dst = reinterpret_cast<Vec *>(y); 
   int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int block_size_t, char *argv[]) {
  ...
  //  int ny=     1024   , nx =   16  ; 
   ...
   int block_4 =  64;
   int grid_4 = (nx / vecsize + block_4 - 1)/ block_4;
  
   int block_1 =  32;
   int grid_1 = (nx + block_1 - 1)/ block_1;
   if (grid_2 <= 0) printf("ERROR\n");
   if (grid_4 <= 0) printf("ERROR\n");
   if (grid_1 <= 0) printf("ERROR\n");
   if (grid <= 0) printf("ERROR\n");
   for (int i = 0; i < 1000; i++) {
     ...
     // cudaDeviceSynchronize();
     // gpuReduce_block_size_store<<<grid_2, block_num, sizeof(float) * block_size_x * block_num>>>(x_d, y_d, nx, ny);
     // cudaDeviceSynchronize();
     // gpuReduce_block_size_storeAll<<<grid_2, block_num>>>(x_d, y_d, nx, ny);
     // cudaDeviceSynchronize();
     // gpuReduce_block_size_loadAndStore<<<grid_2, block_num>>>(x_d, y_d, nx, ny);
     // cudaDeviceSynchronize();

   gpuReduce_vec_4<<<grid_4, block_4>>>(x_d, y_d, nx, ny);
   cudaDeviceSynchronize();
   gpuReduce_vec_store<<<grid_4, block_4>>>(x_d, y_d, nx, ny);
   cudaDeviceSynchronize();
   gpuReduce_vec_storeAll<<<grid_4, block_4>>>(x_d, y_d, nx, ny);
   cudaDeviceSynchronize();
   gpuReduce_vec_loadAndStore<<<grid_4, block_4>>>(x_d, y_d, nx, ny);
   cudaDeviceSynchronize();
