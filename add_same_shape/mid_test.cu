/*
 reduce higherDim 性能验证，用于确定block拆分以及最合适的kernel拆分方式
*/
...
__global__ void gpuReduce_y(float*x, float* y, int nx, int ny, int block_size_reduce_ny) {
   ...
   if (idx > nx) return;
   for(int l = 0; l < hl ; l++) {
     for (int iy = 0; iy < block_size_reduce_ny && idy + iy < ny; iy ++) {
        int id = (idy + iy) * nx + idx + l * ny * nx;
        float tp = x[id];
        temp += tp;
     }
     y[idx + blockIdx.y * nx + l * gridDim.y * nx] = temp;
   }
}
...
//ny = ny * 2; 
   ...
   int block_size_reduce_ny = GetLastPow2(ny / num_block);
...
// int grid_y = (ny + BLOCK_SIZE - 1)/BLOCK_SIZE;
   ...
   for (int i = 0; i < 1000; i++) {
    ...
    //  cudaDeviceSynchronize();
     ...
     if(grid_y > 1) {
       dim3 temp = (grid_3, 2);
       gpuReduce_global<<<temp, block_1>>>(y_d, y_d, nx, grid_y);
      // gpuReduce_global_vec_4<<<grid5.x, block5>>>(x_d, y_d, nx/4, grid5.y);
     }
   }

   if (grid_y) {
     printf("block_y %d block_x %d grid_x %d grid_y %d", grid3.x, grid3.y, block3.x, block3.y);
   }
   printf("grid_a %d\n", grid_y);
   cudaDeviceSynchronize();
