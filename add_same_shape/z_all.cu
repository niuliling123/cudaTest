...
__global__ void gpuReduce_base(float*x, float* y, int nx, int ny) {
   ...
   if (idx < nx) {
     for (int iy = 0; iy < ny; iy ++) {
        int id = iy * nx + idx + blockIdx.y * ny * nx;
        float tp = x[id];
        temp += tp;
     }
...
// ny = ny * 2; 
   int num = 2 * 4 * 6 * 8;
   std::vector<int> x_dim = {2, 4, 6, 8};
   std::vector<int> y_dim = {1, 3};
   // int num = nx * ny * nz;
   ...
   for (int loop = 0; loop < y_dim.size(); loop++) {
     nz = 1;
     for(int zn = 0; zn < y_dim[loop]; zn++) {
        nz *= x_dim[zn];
     }
     ny = x_dim[y_dim[loop]];

     nx = 1;
     for (int zn = y_dim[loop] + 1; zn < x_dim.size(); zn++) {
        nx *= x_dim[zn];
     }
     printf("nz = %d, ny = %d , nx = %d\n", nz, ny, nx); 
     int block = 32;
  ...
  // int grid_y = (ny + BLOCK_SIZE - 1)/BLOCK_SIZE;
     ...
     for (int i = 0; i < 1; i++) {
       if (grid_y == 1)  {  
         dim3 pp(grid_0, nz);
         dim3 tt(block_1, 1);
         gpuReduce_base<<<pp, tt>>>(x_d, y_d, nx, ny);
